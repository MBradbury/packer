...
You are recommended to use the [-NonInteractive parameter](https://learn.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_powershell_exe?view=powershell-5.1#-noninteractive)
to ensure commands that require interaction result in an error
instead of hanging while waiting for input.
...
provisioner "shell-local" {
  ...
  execute_command  = ["powershell.exe", "-NonInteractive", "{{.Vars}} {{.Script}}"]
